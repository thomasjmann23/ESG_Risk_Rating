...
def main():
    ...
    cols_per_row = 2
    for i in range(0, len(project_data[:8]), cols_per_row):  # Show first 8 projects
        cols = st.columns(cols_per_row)
        for j in range(cols_per_row):
            if i + j < len(project_data[:8]):
                project = project_data[i + j]
                risk_class = get_risk_class(project['risk_level'])
                status_class = get_status_class(project['risk_level'])
                
                with cols[j]:
                    st.markdown(f"""
                    <div class="project-card {risk_class}">
                        <h4 style="margin: 0 0 0.5rem 0; color: #1f2937;">{project['project_name']}</h4>
